...
function checkLimits(directory: string): { withinLimits: boolean; violations: string[] } {
  ...
  if (metrics.iterationCounts.total > metrics.limits.maxIterations) {
    violations.push(`Total iterations (${metrics.iterationCounts.total}) exceeds limit (${metrics.limits.maxIterations})`)
  }
...
Details: [your specific request]`,
      ...
      documentationLinks: [
        "- template/agents/idumb-supreme-coordinator.md (your profile)",
        "- template/router/chain-enforcement.md (delegation rules)",
...
Verify: Read file first, commit after`,
      ...
      documentationLinks: [
        "- template/agents/idumb-high-governance.md (your profile)",
        "- template/router/chain-enforcement.md (coordination rules)",
...
Recommendation: Delegate to @idumb-builder for fix`,
      ...
      documentationLinks: [
        "- template/agents/idumb-low-validator.md (your profile)",
        "- template/router/chain-enforcement.md (validation rules)",
    ...
    'idumb-builder': {
      ...
      documentationLinks: [
        "- template/agents/idumb-builder.md (your profile)",
...
/**
 * Check if state is stale (> 48 hours since last validation or update)
 */
function isStateStale(directory: string): { stale: boolean; hours: number } {
  const state = readState(directory)
  if (!state) return { stale: true, hours: Infinity }
...
function buildPrerequisiteGuidance(failure: string): string {
  ...
  if (failure.includes("state.json")) {
    ...
    📖 See: template/router/chain-enforcement.md (INIT-01)`
  }
  ...
  if (failure.includes("PROJECT.md")) {
    ...
    📖 See: template/router/chain-enforcement.md (PROJ-01)`
  }
  ...
  if (failure.includes("ROADMAP.md")) {
    ...
    📖 See: template/router/chain-enforcement.md (PROJ-02)`
  }
  ...
  if (failure.includes("PLAN.md")) {
    ...
    📖 See: template/router/chain-enforcement.md (PHASE-01)`
  }
  ...
  if (failure.includes("CONTEXT.md")) {
    ...
    📖 See: template/router/chain-enforcement.md (PHASE-02)`
  }
  ...
  if (failure.includes("VERIFICATION.md") || failure.includes("verification evidence")) {
    ...
    📖 See: template/router/chain-enforcement.md (VAL-01)`
  }
  ...
  if (failure.includes("execution evidence")) {
    ...
    📖 See: template/router/chain-enforcement.md (PHASE-03)`
  }
  
  return `
    📋 To satisfy: Review chain rules
    Command: /idumb:help
    Or: Read template/router/chain-enforcement.md
    
    📖 See: template/router/chain-enforcement.md`
}
...
function buildChainBlockMessage(
  ...
  lines.push("")
  lines.push("📖 Documentation: template/router/chain-enforcement.md")
  ...
  return {
    ...
    "experimental.chat.messages.transform": async (input: any, output: any) => {
      try {
        ...
        // Enhanced compaction detection with multiple indicators
        ...
        if (isCompacted) {
          const detectionReason = hasCompactionKeyword 
            ...
            : hasContextLossIndicator 
              ...
              : 'low_message_count'
          log(directory, `Compaction detected via: ${detectionReason}`)
        }
