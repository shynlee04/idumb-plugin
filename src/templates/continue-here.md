...
- `.idumb/idumb-brain/state.json` - Full state
- `.planning/phases/{N}/` - Phase artifacts
- `.idumb/idumb-brain/execution/{N}/progress.json` - Execution progress
...
content_sources:
  current_state:
    from: ".idumb/idumb-brain/state.json"
  ...
  preserved_context:
    from: ".idumb/idumb-brain/state.json → anchors"
    filter: "priority in ['critical', 'high']"
    
  in_progress:
    from: ".idumb/idumb-brain/execution/{N}/progress.json"
