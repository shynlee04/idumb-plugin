...
> **Note:** Planning methodology is in `agents/idumb-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @src/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/idumb:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.

**Must-haves enable verification:** The `must_haves` field carries goal-backward requirements from planning to execution. After all plans complete, `@idumb-verifier` checks these criteria against the actual codebase.
...
Wave 3 runs after Waves 1 and 2. Pauses at checkpoint, `@idumb-executor` presents to user, resumes on approval.
...
See `src/references/tdd.md` for TDD plan structure.
...
| `checkpoint:decision` | Implementation choices | Pauses, returns to user |
| `checkpoint:human-action` | Truly unavoidable manual steps (rare) | Pauses, returns to user |

**Checkpoint behavior in execution:**
- Plan runs until checkpoint
- `@idumb-executor` pauses, creates anchor in state.json
- User responds
- `@idumb-executor` resumes execution
...
must_haves:
  ...
  key_links:
    - from: "src/features/user/UserList.tsx"
      to: "/api/users"
      via: "fetch"
---
...
The `must_haves` field defines what must be TRUE for the phase goal to be achieved. Derived during planning, verified after execution by `@idumb-verifier`.
...
**Verification flow (iDumb):**

1. `@idumb-planner` derives must_haves from phase goal (goal-backward)
2. Must_haves written to PLAN.md frontmatter
3. `@idumb-executor` runs all plans
4. `@idumb-verifier` checks must_haves against codebase
...
| Agent | Role in Plan Lifecycle |
|-------|------------------------|
| `@idumb-planner` | Creates PLAN.md with must_haves |
| `@idumb-plan-checker` | Validates plan before execution |
| `@idumb-executor` | Executes tasks, handles checkpoints |
| `@idumb-builder` | Actually writes files (delegated by executor) |
| `@idumb-verifier` | Validates must_haves after completion |
