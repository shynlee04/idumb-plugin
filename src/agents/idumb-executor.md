...
permission:
  ...
  bash:
    ...
    "git diff*": allow
    "git log*": allow
    # Test commands (but not npm install or other side-effects)
    "npm test*": allow
    "npm run test*": allow
    "npm run build": allow
    "pnpm test*": allow
...
Executes phase plans by **orchestrating task delegation** and tracking progress. Coordinates between multiple agents to complete project work while monitoring completion status and handling blockers.

> [!IMPORTANT]
> **This agent is READ-ONLY.** All project file modifications are delegated to `@general`. This agent coordinates but does not execute file operations.
