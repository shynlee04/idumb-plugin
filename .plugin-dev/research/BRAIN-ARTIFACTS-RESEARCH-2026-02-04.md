...
|----------|-----------|---------|----------|
| **config.json** | `ensureIdumbConfig()` | session.created hook | `.idumb/config.json` |
...
| **sessions/*.json** | `storeSessionMetadata()` | session.created | `.idumb/sessions/` |
| **halt-*/halt-context.json** | `triggerEmergencyHalt()` | MAX_DELEGATION_DEPTH, MAX_ITERATIONS | `.idumb/execution/halt-{timestamp}/` |
| **timestamps/*.json** | `storeTimestamp()` | file modifications | `.idumb/timestamps/` |
| **plugin.log** | `log()` function | all plugin activity | `.idumb/governance/plugin.log` |
| **validations/*.json** | Manual validation runs | /idumb:validate, agent checks | `.idumb/governance/validations/` |
...
function storeSessionMetadata(directory: string, sessionId: string): void {
  // Creates: .idumb/sessions/{sessionId}.json
}
...
function triggerEmergencyHalt(reason, context, directory): void
  // Creates: .idumb/execution/halt-{timestamp}/halt-context.json
...
| `checkpoint-schema.json` | `.idumb/execution/*/halt-context.json` | ✅ Defined |
...
garbage_collection:
  auto_delete:
    - pattern: ".idumb/sessions/*.json"
      condition: "Older than 7 days"
      action: DELETE
      
    - pattern: "*.log"
      location: ".idumb/governance/"
